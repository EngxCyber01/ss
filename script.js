...
ctaButtons.forEach(button => {
    button.addEventListener('click', (e) => {
        if (button.textContent.includes('Started')) {
            ...
            window.open('Contact.html', '_blank'); // Open in new tab
        }
    });
});
});
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Trigger navbar animation on scroll
...
window.addEventListener('scroll', () => {
    // Don't show scroll-to-top button on socialmedia.html
    const currentPage = window.location.pathname.toLowerCase();
    if (currentPage.includes('socialmedia.html')) {
        return;
    }
    ...
    if (window.pageYOffset > 300) {
        if (!document.querySelector('.scroll-top')) {
            ...
            scrollTopBtn.style.cssText = `
                ...
                5px 5px 7px rgb(0, 0, 0,0.65) inset,
                -5px -5px 7px rgba(255,255,255,0.75) inset;
                animation: slideUp 0.3s ease;
                transition: all 0.3s ease;
            `;
            
            // Add media query for mobile positioning
            ...
            scrollTopBtn.addEventListener('mouseenter', () => {
                ...
                scrollTopBtn.style.backgroundColor ='#093961';
                scrollTopBtn.style.color ='#fff';
                scrollTopBtn.style.boxShadow = '0 0 30px rgba(0, 212, 255, 0.5),5px 5px 7px rgb(255,255,255,0.55) inset, -5px -5px 7px rgba(255,255,255,0.65) inset';
            });
            
            scrollTopBtn.addEventListener('mouseleave', () => {
                scrollTopBtn.style.transform = 'scale(1)';
                scrollTopBtn.style.boxShadow = '0 0 20px rgba(0, 212, 255, 0.3),5px 5px 7px rgb(0, 0, 0,0.65) inset,-5px -5px 7px rgba(255,255,255,0.75) inset';
                scrollTopBtn.style.background='var(--primary-color)';
                scrollTopBtn.style.color ='#000000ff';
            });
        }
    } else {
